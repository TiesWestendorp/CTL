...
for(let i = 0; i < n; i++) {
  ...
  transitions.push(bdd.andN([
    bdd.andN(indices_except_i.map(j => bdd.eql(left_utensils[j][0],  left_utensils[j][1]))),
    bdd.andN(         indices.map(j => bdd.eql(right_utensils[j][0], right_utensils[j][1]))),
    bdd.andN(         indices.map(j => bdd.eql(eating[j][0],         eating[j][1]))),
  ...
  transitions.push(bdd.andN([
    bdd.andN(         indices.map(j => bdd.eql(left_utensils[j][0],  left_utensils[j][1]))),
    bdd.andN(indices_except_i.map(j => bdd.eql(right_utensils[j][0], right_utensils[j][1]))),
    bdd.andN(         indices.map(j => bdd.eql(eating[j][0],         eating[j][1]))),
  ...
  transitions.push(bdd.andN([
    bdd.andN(indices_except_i.map(j => bdd.eql(left_utensils[j][0],  left_utensils[j][1]))),
    bdd.andN(indices_except_i.map(j => bdd.eql(right_utensils[j][0], right_utensils[j][1]))),
    bdd.andN(indices_except_i.map(j => bdd.eql(eating[j][0],         eating[j][1]))),

    left_utensils[i][0], right_utensils[i][0], bdd.not(eating[i][0]),
    left_utensils[i][1], right_utensils[i][1], eating[i][1]
  ]))
...
console.warn("Does there exist a loop that avoids deadlocks, where each philosopher gets to eat infinitely often?", bdd.and(initial, fair_ctl.EG(bdd.not(deadlocks), fairness, transition)).isSatisfiable)
